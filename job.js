...
// scrapePerDate("UCt4t-jeY85JegMlZ-E5UWtA", new Date(new Date().setDate(new Date().getDate() - 12)).toISOString(), new Date().toISOString())
//2010-04-29T10:54:00.000Z
// scrape("UCt4t-jeY85JegMlZ-E5UWtA")
// youtubeSearch("Aaj Tak")
//144556
async function youtubeSearch(val) {

    // return { "hai": "sa " }
    try {
        const hasil = await axios.get(`https://content.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(val)}&type=channel&key=AIzaSyAa8yy0GdcGPHdtD083HiGGx_S0vMPScDM`, {
            headers: {
...
async function scrapePerDate(id, publishedAfter, publishedBefore) {
    let panda = []
    const axios = require("axios")
    ...
    for (let i = 0; i < countWord.models.length; i++) {
        ...
        for (let j = 0; j < semua.length; j++) {
            if (saveWord[semua[j]] === undefined) {
                saveWord[semua[j]] = { word: semua[j], translated: "", count: 1, title: [], parsedTitle: [], videoId: [], channelId: "" }
